...
def get_frequent_indicator_codes(
    ...
    """
    Query indicator codes and names with at least `min_rows` entries in `target_year`.
    """
    query = f"""
        SELECT 
            indicator_code, 
...
def main():
    ...
    print("\n[2/3] Top Countries by Education Spending (% of GDP, 2010-2017):")
    spending_df = get_top_education_spending_countries(client)
    print(spending_df.head(10).to_string())

    print("\n[3/3] Frequently Reported Indicator Codes (>= 175 entries in 2016):")
